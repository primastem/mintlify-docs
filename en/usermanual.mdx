...
- First run the "Calibration" card without a number: the robot measures motor polarity, removes backlash and sets up straight-line movement.
- Then calibrate the turn angles. Mark the exact orientation of the robot on the table surface. You need 16 turns of 90° clockwise: build the program "Repeat 8 + Right turn" and run it twice. The robot should make four full rotations (90×16 = 1440°) and return to its original orientation.
- Assess the deviation from the mark and apply a correction with the "Calibration" card plus a number chip: if the robot stops before the mark, add "+" (for example "Calibration" + "+5"); if it goes past, add "−" (for example "Calibration" + "−5"), then "Execute".
- Repeat until the robot returns exactly to its original orientation.
- The calibration is saved after power-off.
...
Store, use and charge the set in a dry room at a temperature of \+5…\+35°C. Avoid direct sunlight, liquids, dust, and sudden changes of temperature or humidity that can cause condensation.
