...
A marker with a diameter of up to 10 мм can be installed in the center of the robot to draw simple shapes while moving.
...
The remote control contains 11 dual slots for inserting command and value chips: 6 for the main program (upper part), 5 for the subroutine (lower part).
The remote has two buttons: on the left — power, on the right — "Execute / Stop" for starting and stopping the program.
...
The set includes chips for creating programs.
Each chip is a command with a clear meaning and instruction. The sequence of blocks determines the robot's behavior.
...
- **Forward** — forward movement (15 см by default)
- **Right** — 90° clockwise turn
- **Left** — 90° counterclockwise turn
- **Backward** — backward movement (15 см by default)
...
Angle and distance chips: 30°, 36°, 45°, as well as multiples (60°, 72°, etc.).
Angles in degrees, distances in millimeters.
By default — 90° and 100 мм (15 см).
...
To program movement, place command chips in the remote slots (for example, "Forward", "Left", "Function", etc.).
You can set command values and repeat counts, as well as angle, distance, and arithmetic operation values.
...
- Change the "**default distance**" using a service chip: for example, for 12.5 см, place the "Default Distance" chip with value 125. The value is saved after power-off.
...
<Accordion icon="rectangle-terminal" title="Calibration of the first device version (MVP) with 10 см diameter robot">
...
2. To calibrate the movement distance, measure the actual path with the "Forward" command, place "Calibration" and the chip with the required value, then "Execute". You can write the value to any digital NFC chip using a mobile phone with NFC as text in the form nXXX (for example, n095 for 95 мм). After this, the robot's movement distance will be accurate.*
</Accordion>
