...
The activities were developed for various goals and educational contexts. The guide presents numbered activities.
Below we recommend activities based on your conditions.
...
In a lesson for working with PrimaSTEM, it is ideal to form small groups of 4-6 children with one teacher.
At this time, other students can work on another task with a teacher's assistant or, for example, practice drawing the robot's path according to the program (for 1st-2nd grade students).
In both cases, it is preferable to conduct PrimaSTEM sessions in a multipurpose hall, right on the floor or by pushing tables and chairs aside to clear space.
...
| ------- | ----------------------------------- | -------------------------------------------------------------------------------------------- |
| Forward   | ![](../../images/book-1/forward.excalidraw.svg)  | The robot advances one cell — a logical step (by default equal to 15 cm)              |
| Left  | ![](../../images/book-1/left.excalidraw.svg)     | The robot turns 90° left                                                           |
| Right | ![](../../images/book-1/right.excalidraw.svg)    | The robot turns 90° right                                                          |
| Back   | ![](../../images/book-1/back.excalidraw.svg)     | The robot drives back one cell — a logical step by default                                |
| Function | ![](../../images/book-1/function.excalidraw.svg) | The robot executes the sequence of commands located in the function line                    |
| Repeat № | ![](../../images/book-1/repeatN.excalidraw.svg)  | The robot repeats the command set in the paired cell a certain number of times — №      |
...
When you insert a token into one of the slots and it is correctly positioned, the LED lights up green.
When adding a repeat command to a movement command in a paired slot, the LED lights up blue.
If tokens are installed incorrectly (for example, two movement command tokens in a paired slot), a red LED lights up. In this case, the erroneous command will be ignored when the program is executed.
...
When the robot or remote control is turned on, a short beep sounds.
When the remote control is on, the LED on the front panel lights up green. When the robot is on, the LED near the USB-C connector lights up green.
...
Video instructions are available on our YouTube channel — https://www.youtube.com/@primastem
...
> Understand the robot's movement capabilities
> Learn that the remote control controls the robot
> Understand command tokens
...
| Image                                        | Description                                                                    |
| -------------------------------------------------- | --------------------------------------------------------------------------- |
| ![](../../images/book-1/forward.excalidraw.svg) | The **"Forward"** command token makes the robot move one cell |
...
| Image                                     | Description                                                                                                                                                                                |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![](../../images/book-1/left.excalidraw.svg) | The token with an arrow turning in an arc around the center is **"Left"** — it makes the robot turn left, counterclockwise. The default rotation step is 90 degrees. |
...
| Image                                      | Description                                                     |
| ------------------------------------------------ | ------------------------------------------------------------ |
| ![](../../images/book-1/right.excalidraw.svg) | The **"Right"** token makes the robot turn right. |
...
> Consider the robot's orientation at the beginning of the program
> Anticipate robot movements depending on the program
...
No indications are given as to which direction the child-robot is facing.
Determine which direction of the robot is correct for completing the mission.
...
> Consider the robot's initial orientation
...
Here is the same program, the robot will also advance three times straight.
Only this time it is not oriented the same way at the beginning: it is looking to the right. Therefore, it will advance three times to the right:
...
This exercise on paper can be difficult.
If some children find it hard to understand robot rotations and sequences of movements, take the PrimaSTEM play set and ask them to reproduce the programs that are on the sheets on the remote control.
By manipulating and observing, we understand better!
...
> Establish a connection between instructions and movements performed by the robot
> Visualize map boundaries
> Be able to transfer command instructions to the "control panel"
...
> Decompose the route into stages
> Implement a program with a specific goal
...
Should it go straight, turn left, turn right?
Show them what movement each command will produce on the robot, moving it across the map with your hands.
...
> Understand that the "function" command can replace other command instructions
> Consider the concept of repeating a sequence of instructions
...
When the "Function" command is used, everything happens as if we had put what is inside the [-----] frame at the bottom of the control panel in place of the "Function" token.
In our example, the robot will advance twice straight.
...
And for the final: **the cycle**!
Reproduce the program with four "Function" blocks, as in the figure below.
...
> Decompose the route into stages.
> Implement a program with a specific goal.
> Use a "function" block in the program.
...
Does the robot reach the "Night" cell? If not, why?
Allow the children to try to fix their errors in the program, if any.
...
> Find an error in the program by observing its execution.
> Fix an error in the program.

For this activity, form small groups of students. Sit at a low table or on the floor. Prepare the grid field, the remote control, the robot, and the tokens.
Ask groups of students to debug the program in turn.
...
> Anticipate robot movements to find an error in the program.
> Understand how to fix an error in the program.
...
Once the path is drawn, we see that the robot is not going to the finish (black)! It arrives at the red flag.
Ask the children where the robot went wrong. Here you need to find the cell that contains the error, circled in red in the correction.
...
Once we have placed our bets, we launch the program to check what happens.
Then we put the robot back in the corner, and the next child starts their program.
...
To learn more about the PrimaSTEM device, visit the documentation resource site available in 10 languages: https://docs.primastem.com

### Have questions? Contact us!

Don't hesitate to contact us: primastem@gmail.com

Our website with information and links to social media: https://primastem.com
