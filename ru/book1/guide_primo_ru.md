...
PrimaSTEM позволяет шаг за шагом изучить логическое мышление, основы программирования и математики. Это руководство поможет вам проводить занятия с PrimaSTEM.

### Несколько слов об устройстве

Цель этого устройства — программировать движения маленького робота - Божью коровку с помощью фишек - команд, которые размещаются на пульте дистанционного управления.

Команда "Вперёд" позволяет двигаться прямо, "Влево" — поворачивать налево, "Вправо" — поворачивать направо. Фишка "[ ]" позволяет создать функцию, то есть заменить последовательность команд. "Повтор" - повторить команду или функцию несколько раз. Мы размещаем команды на пульте, чтобы создать программу, которая укажет роботу как ему двигаться.
...
- Понимание того, что такое «баг» - ошибка в коде, и умение находить и исправлять ошибки в программе
...
Уроки были разработаны для различных целей в разнообразных образовательных контекстах. Буклет предлагает работу по пронумерованным видам деятельности.
...
Рекомендуем использовать одно устройство на 2-3 ученика для обучения в команде и построения социальных связей.

### ШКОЛЬНАЯ ДЕЯТЕЛЬНОСТЬ

На уроке для работы с PrimaSTEM идеально формировать небольшие группы по 4 или 6 детей с одним учителем.
В это время другие ученики могут работать над другим упражнением с помощником воспитателя или, например, тренироваться рисовать путь робота по программе для учеников 1-го/2-го классов.
...
- **Прямо:** Робот продвигается на одну клетку (по умолчанию на 15 см.)
...
- **Повтор №** Робот выполняет повтор команды установленной в парной ячейке определённое количество раз

### Пульт дистанционного управления

Он позволяет управлять роботом, располагая блоки инструкций - команды (фишки) в доступные ячейки (D). 
6 верхних, двойных ячеек (A), соединенных линией выполнения (E), составляют основную последовательность программы. Программа начинается с ячейки, расположенной слева (D), и заканчивается над кнопкой START (B), которая позволяет отправить инструкции роботу и запустить программу. 
...
Когда вы вставляете фишку в одну из ячеек и она правильно расположена, светодиод загорается зелёным светом. При добавлении значения или повтора в парную ячейку светодиод загорается синим цветом. При неправильной установке фишек загорается красный диод, в этом случае ошибочная команда при выполнении программы будет игнорироваться.

### Аккумуляторная батарея и кнопка ON/OFF

Робот и пульт дистанционного управления питаются от встроенного аккумулятора каждый. Они заряжаются через порт USB-C. У них обоих есть кнопка ON/OFF, которая находится сверху на роботе и на передней стороне пульта. Когда робот или пульт включается, он издает короткий звуковой сигнал. Когда пульт включен, его светодиод на передней панели горит зелёным светом. Когда робот включен, его светодиод около разъёма USB-C горит зелёным светом. 

### Беспроводная связь

Робот и пульт дистанционного управления общаются по беспроводной связи Bluetooth с радиусом действия около 10 м. Беспроводная система работает в фоновом режиме, без настройки, после первого подключения робота к пульту. 
Пульт можно настроить на работу с другим роботом (сделать сопряжение устройств), включив не подключенного к пульту робота и включив пульт и удерживая на нем нажатой кнопку START более 10 секунд.

Последовательность движений, или «программа», начинается слева на пульте и следует по выгравированной последовательности из стрелок ">", соединяющей парные ячейки. 
После того как последовательность установлена, нажимается кнопка "START" для запуска программы. 
Если фишка добавлена после пустого отверстия, соответствующая ей инструкция будет выполнена после пропуска пустой ячейки.
Если выставлена неправильная фишка или комбинация фишек в парной ячейке, то светодиод загорится красным, и такая команда будет пропущена - проигнорирована при выполнении программы.
...
Блок "[ ]", также называемый блоком «функция», используется для замены последовательности движений.

Зеленый блок позволяет выполнять более сложные последовательности и идти дальше в решении проблем. Чтобы создать функцию, достаточно вставить последовательность движений в специально отведенное для этого поле - 5 сдвоенных ячеек в нижней части пульта. Эта последовательность выполняется слева направо каждый раз, когда в основной последовательности встречается зеленый блок.
...
Робот для начальных занятий -обучению алгоритмам, программированию и числам может перемещается по карте с сеткой. Рекомендуется использовать карты с клеткой размером 15 см: это расстояние, которое робот проходит за один шаг прямо. Карта-шахматная доска используется для отработки перемещения из одной точки в другую, без отвлечения на цвета. Чтобы рассказывать небольшие истории о перемещениях робота, можно использовать цветную карту. Например, «Божья коровка выходит из дома и идет в горы через лес». Можно представить, что класс работает над созданием новой карты, чтобы рассказывать новые истории о путешествиях божьей коровки.

Возможно изменить расстояние движения по умолчанию с 15 см. на любой, под имеющуюся карту - 10 см. или 12.5 см. или 20 см. с помощью специальной команды устанавливающей значение шага + число в мм (для 10 см. нужно использовать 100, для 12,5 см. нужно использовать 125)
